...
//        }
     ...
     * @param obj
     * @param value
     * @param manyToOneFlag 多对一标志（true时检测多对一，false仅赋值）
     * @param <T>
     * @Author 陈文
     * @Date 2019/12/27  10:44
     * @Description 将数据库查询值映射进实体
     */
    public static <T> T setValue(Class<T> cls, Field field, T obj, Object value, boolean manyToOneFlag) {
        try {
            ManyToOne manyToOne = manyToOneFlag ? getManyToOne(cls, field) : null;
            if (manyToOne != null) {
                ...
                if (fieldValue != null) {
                    ...
                    fieldValue = setValue(fieldClass, fieldChild, fieldValue, value, true);
...
//                e.printStackTrace();
    ...
    public static String getId(Object t) {
        try {
            ...
            String getField = ClassUtil.getField(fieleName);//"getId";
     ...
     * @date 2019/12/11 16:57
     */
    public static <T> Object getValue(T t, Field field, Class<T> cls) {
        return getValue(t, field.getName(), cls);
    }
     ...
     * @date 2019/12/11 16:57
     */
    public static <T> Object getValue(T t, String fieldName, Class<T> cls) {
        try {
            Method getMethod = ClassUtil.getGetMethod(cls, fieldName);
    ...
    private static Object getPrimaryValue(Class cls, Object t, String fieldName) {
        try {
            Method getMethod = ClassUtil.getMethod(cls, ClassUtil.getField(fieldName));
            Object value = getMethod.invoke(t);
            if (value != null) {
                return value + "";
            }
...
//    }
        ...
        for (Field field : fields) {
            try {
                ...
                if (value != null) {
                    // 声明类函数方法，并获取和设置该方法型参类型
                    Class type = field.getType();
                    Method setMethod = ClassUtil.getSetMethod(cls, field, type);
    ...
    public static ManyToOne getManyToOne(Class<? extends Object> cls, Field field) {
        try {
            Method getMethod = ClassUtil.getGetMethod(cls, field);
